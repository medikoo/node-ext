...
filterByType = function (paths, types, map) {
	...
	promise = deferred(paths, map)(function (data) {
		return (result = data[0].filter(function (path) {
			return contains.call(types, this[path]);
		}, data[1]));
	});
	return defineProperty(promise, 'root', d(root));
};

filterByPattern = function (paths, pattern) {
...
filterIgnored = function (paths, rules) {
...
readDeep = function (path, depth, type, pattern, ignoreRules) {
	...
	if (type) {
		...
		paths = filterByType(paths, type, typesMap);
	}
	if (pattern) {
