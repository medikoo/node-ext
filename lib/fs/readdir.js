...
var isArray         = Array.isArray
  ...
  , toFullPath, read, readDeep, filterByTypeAndPattern, filterByPattern
  , filterIgnored, getType, dirType, getTypesMap;
...
getTypesMap = function (paths, type1, type2, pattern) {
	...
	paths.end(function (paths) {
		...
		paths.forEach(function (path) {
			lstat(root + sep + path, function (err, stat) {
				...
				if (type1[type] && (!pattern || pattern.test(root + sep + path))) {
					list1.push(path);
				}
				if (type2 && type2[type]) {
					list2.push(path);
...
filterByTypeAndPattern = function (paths, types, map, pattern, watch) {
	...
	if (watch) {
		test = function (path) {
			return pLstat(root + sep + path)(function (stat) {
				return (types[getTypeFromStat(stat)] &&
					(!pattern || pattern.test(root + sep + path))) ? path : null;
			});
		};
...
filterIgnored = function (paths, rules, watch) {
	...
	test = function (path) {
		var status = isIgnored(rules, root + sep + path, { watch: watch });
		if (watch) {
...
readDeep = function (path, depth, type, pattern, ignoreRules, watch) {
	...
	if (type) {
		isTypeDirType = (isCopy(dirType, type) && !pattern) || !depth;
		typesMap = getTypesMap(paths, type, isTypeDirType ? null : dirType,
			pattern);
		paths = filterByTypeAndPattern(paths, type, typesMap.get(0), pattern,
			watch);
		if (isTypeDirType) {
			dirPaths = paths;
		}
	...
	if (depth) {
		if (!isTypeDirType) {
			dirPaths = filterByTypeAndPattern(dirPaths, dirType,
