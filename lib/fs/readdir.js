...
var isArray         = Array.isArray
  ...
  , native = fs.readdir, lstat = fs.lstat, pLstat = promisify(lstat)
...
Readdir.prototype = {
	init: function () {
		var data, result, promise, emptyArr, progress;
		...
		(function self(data, root, depth) {
			...
			this.readers.push(data.files);
			if (data.dirs && (data.dirs !== data.files)) {
				this.readers.push(data.dirs);
			}
			if (root) {
			...
			if (this.watch) {
				if (root) {
					data.files.on('end', function (files) {
						remove.call(this.readers, data.files);
						if (data.dirs && (data.dirs !== data.files)) {
							remove.call(this.readers, data.dirs);
						}
						if (files.length) {
							files = files.map(getPath);
	...
	close: function () {
		if (this.readers) {
			forEach(this.readers, invoke('close'));
			delete this.readers;
		}
	},
	...
	filterByType: function (paths, getDirs) {
		var promise, result = {}, test, root = paths.root
		  , files, dirs, resolve, defFiles, defDirs, close;
	...
	readdir: function (path) {
		...
		if (this.watch) {
			...
			watcher.on('end', function () {
				delete this.readers[path];
				promise.emit('end', files);
			}.bind(this));
			watcher.on('change', function () {
				native(path, function (err, data) {
					var removed, added;
					if (err) {
						promise.emit('end', files);
						return;
...
readdir = function (path, options) {
	var depth, type, pattern, ignoreRules, watch, readdir
	  , ignoreMapGetters, globalRules, buildMap;
	...
	if (options.ignoreRules) {
		...
		ignoreRules.forEach(function (name) {
			...
			if (watch) {
				isRoot = memoizeWatcher(mode.isRootWatch);
				readRules = memoizeWatcher(getConfFileMap.readRulesWatch);
			} else {
				isRoot = memoize(mode.isRoot);
				readRules = memoize(getConfFileMap.readRules);
			}
			ignoreMapGetters.push(function (path) {
				var map, finder;
		...
		buildMap = (watch ? memoizeWatcher : memoize)(function (dirname) {
			return ignoresBuildMap(dirname, ignoreMapGetters, watch);
		});
	}
	...
	readdir.watch = watch;
	readdir.progress = Boolean(options.progress);
