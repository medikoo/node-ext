...
var isArray         = Array.isArray
  ...
  , Readdir, readdir, enoentSurpass, passErrCodes;
...
Readdir.prototype = {
	init: function () {
		...
		(function self(data, root, depth) {
			...
			.end(this.resolve.bind(this, result), this.resolve);
	...
	filterByType: function (paths, getDirs) {
		...
		resolve = function (e) {
			// log("READDIR", root, "TYPE: RESOLVE", e ? "ERROR" : "SUCCESS", e ? e : { files: files, dirs: dirs });
			if (defFiles) {
		...
		paths.end(function (paths) {
			// log("READDIR", root, "TYPE: PARENT SUCCESS");
		...
		if (this.watch) {
			...
			paths.on('end', function (data, err) {
				var e;
				// log("READDIR", root, "TYPE: PARENT END", resolved, failed);
				if (!resolved) {
					if (defFiles) defFiles.resolve(err);
					if (defDirs) defDirs.resolve(err);
					return;
				}
				if (!failed) {
					if (files) {
						result.files.emit('end', files, err);
					}
					if (getDirs) {
						result.dirs.emit('end', dirs, err);
					}
				}
			});

			close = function () {
				// log("READDIR", root, "TYPE: FORCE CLOSE", resolved, failed);
				if (defFiles && !defFiles.resolved) {
					defFiles.resolve(new Error("Readdir operation cancelled"));
				}
				if (defDirs && !defDirs.resolved) {
					defDirs.resolve(new Error("Readdir operation cancelled"));
				}
				paths.close();
			};
	...
	filterByPattern: function (paths) {
		...
		// log("READDIR", root, "PATTERN: START");
		...
		promise = paths(function (data) {
			// log("READDIR", root, "PATTERN: PARENT SUCCESS");
		...
		if (this.watch) {
			...
			paths.on('end', function (data, err) {
				// log("READDIR", root, "PATTERN: PARENT END");
				promise.emit('end', result, err);
			});
			promise.close = function () {
				// log("READDIR", root, "PATTERN: FORCE CLOSE");
				paths.close();
			};
		}
		return promise;
	...
	filterIgnored: function (paths) {
		var promise, result, test, listener, root = paths.root
		  , promises, def = deferred();

		// log("READDIR", root, "IGNORED: START");
		promise = def.promise;
		...
		if (this.watch) {
			...
			paths.on('end', function (data, err) {
				// log("READDIR", root, "IGNORED: PARENT END", def.resolved);
			...
			promise.close = function () {
				// log("READDIR", root, "IGNORED: FORCE CLOSE", Boolean(promises), def.resolved);
				if (promises) {
					if (!def.resolved) {
						def.resolve(new Error("Operation aborted"));
					}
					forEach(promises, invoke('close'));
					promises = null;
					paths.close();
		...
		paths.end(function (paths) {
			var waiting = paths.length;
			// log("READDIR", root, "IGNORED: PARENT SUCCESS", waiting);
			result = [];
			if (!waiting) {
				def.resolve(result);
				return;
			...
			paths.forEach(function (path) {
				test(path, function (isIgnored) {
					...
					if (!--waiting) {
						// log("READDIR", root, "IGNORED: SUCCESS");
						def.resolve(result);
					}
				});
			});
		}, function (e) {
			// log("READDIR", root, "IGNORED: PARENT ERROR", e);
			def.resolve(e);
		});
		promise.root = root;
		return promise;
	...
	readdir: function (path) {
		...
		promise = def.promise;
		promise.root = path;
		// log("READDIR", path, "BASE: START");
		if (this.watch) {
			try {
				watcher = watchPath(path);
			} catch (e) {
				// log("READDIR", path, "BASE: WATCHER ERROR", e);
				return def.resolve(e);
			}
			watcher.on('end', function (err) {
				// log("READDIR", path, "BASE: WATCHER END", def.resolved);
				if (!def.resolved) {
					def.resolve(err);
				} else if (files) {
			...
			promise.close = function () {
				// log("READDIR", path, "BASE: FORCE CLOSE", def.resolved);
				watcher.close();
				if (!def.resolved) {
					def.resolve(new Error("Readdir action cancelled"));
				}
			};
		}
		original(path, function (err, data) {
			if (err) {
				// log("READDIR", path, "BASE: ERROR", err);
				def.resolve(err);
				return;
			}
			// log("READDIR", path, "BASE: SUCCESS", data);
...
readdir = function (path, options) {
	...
	if (options.ignoreRules) {
		extend(readdir, getIsIgnored(isArray(options.ignoreRules) ?
