...
var isArray         = Array.isArray
  ...
  , IsIgnored       = require('./is-ignored').IsIgnored
...
Readdir = function (path, depth, type, pattern, ignoreRules, watch) {
	this.path = path;
	this.depth = depth;
	this.type = type;
...
Readdir.prototype = {
	...
	filterByType: function (paths, getDirs) {
		...
		if (getDirs) {
			if (this.type && isCopy(this.type, { directory: true }) && !pattern) {
				dirs = files;
			...
			paths.forEach(function (path) {
				lstat(root + sep + path, function (err, stat) {
					...
					if (files && (!this.type || this.type[type]) &&
							(!this.pattern || this.pattern.test(root + sep + path))) {
						files.push(path);
					}
		...
		if (this.watch) {
			test = function (path, files, dirs) {
				...
				return pLstat(fullPath).cb(function (stat) {
					...
					if (files && (!this.type || this.type[type]) &&
							(!this.pattern || this.pattern.test(fullPath))) {
						files.push(path);
					}
...
module.exports = function (path) {
	...
	if (options.ignoreRules) {
		...
		ignoreRules.forEach(function (name) {
			var mode = ignoreModes[name], findRoot, readRules;
			if (!mode) {
				throw new Error("Unknown mode '" + name + "'");
			}
			findRoot = memoize.call(watch ? mode.findRootWatch : mode.findRoot);
			readRules = memoize.call(watch ? getConfFileMap.readRulesWatch :
				getConfFileMap.readRules);
			iGetMap.push(memoize.call(function (path) {
			...
			if (mode.globalRules) {
				push.apply(iGlobalRules, mode.globalRules);
			}
		});
	}
	...
	if (ignoreRules) {
		readdir.isIgnored = function (path) {
			var isIgnored = new IsIgnored(path, watch);
			isIgnored.getMap = iGetMap;
			isIgnored.globalRules = iGlobalRules;
