'use strict';

var isCallable  = require('es5-ext/lib/Object/is-callable')
  , partial     = require('es5-ext/lib/Function/prototype/partial')
  , isString    = require('es5-ext/lib/String/is-string')
  , deferred    = require('deferred')
  , original    = require('fs').readFile
  , resolve     = require('path').resolve
  , watch       = require('./watch').watch
  , WatchPath   = require('./watch-path').WatchPath
  , descLimiter = require('./_descriptor-limiter')

  , readFile, _readFile;

_readFile = function (filename, options) {
	var def, current, promise, watcher, resolve, onchange, loose;

	def = this;
	loose = options.loose;
	descLimiter.open();
	original(filename, options.encoding, resolve = function (err, data) {
		descLimiter.close();
		if (def.resolved) {
			return;
		}
		if (err) {
			if (watcher && !loose) {
				watcher.close();
			}
			def.resolve(loose ? null : err);
			return;
		}
		if (options.watch) {
			current = String(data);
		}
		def.resolve(data);
	});
	promise = def.promise;

	if (options.watch) {
		onchange = partial.call(descLimiter, function () {
			descLimiter.open();
			original(filename, options.encoding, function (err, data) {
				descLimiter.close();
				if (!def.resolved) {
					resolve(err, data);
					return;
				}
				if (!watcher) {
					return;
				}
				if (err) {
					watcher.close();
					promise.emit('end');
					return;
				}
				if (data != current) {
					current = String(data);
					promise.emit('change', data);
				}
			});
		});
		if (loose) {
			current = null;
			watcher = new WatchPath(filename);
			watcher.on('change', function (event) {
				if (event.type === 'remove') {
					if (current != null) {
						promise.emit('change', current = null);
					}
				} else {
					onchange();
				}
			});
		} else {
			try {
				watcher = watch(filename);
			} catch (e) {
				def.resolve(e);
				return;
			}
			watcher.on('change', onchange);
			watcher.on('end', function () {
				watcher = null;
				promise.emit('end');
			});
		}
		return watcher.close;
	}
};

readFile = function (filename, options) {
	var def, sync = true, close, closed = false;

	def = deferred();
	descLimiter(function () {
		if (sync) {
			close = _readFile.call(def, filename, options);
		} else {
			if (closed) return;
			try {
				close = _readFile.call(def, filename, options);
			} catch (e) {
				def.resolve(e);
			}
		}
	});
	sync = false;
	if (options.watch) {
		if (close) {
			def.promise.close = close;
		} else {
			def.promise.close = function () {
				if (close) {
					close();
				} else {
					closed = true;
					def.resolve(new Error("Operation aborted"));
				}
			};
		}
	}
	return def.promise;
};
readFile.returnsPromise = true;

module.exports = exports = function (filename) {
	var def, options, cb, promise, watcher, current;

	filename = resolve(String(filename));
	options = arguments[1];
	cb = arguments[2];
	if ((cb == null) && isCallable(options)) {
		cb = options;
		options = {};
	} else {
		options = isString(options) ? { encoding: options } : Object(options);
	}

	return readFile(filename, options).cb(cb);
};
exports.returnsPromise = true;
exports.readFile = readFile;
