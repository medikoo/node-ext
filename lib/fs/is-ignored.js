...
isIgnored = function (filename, mode, watch) {
...
module.exports = function (mode, path) {
	var globalRules = [], cb, arg, value, watch, options;
	...
	if (arg) {
		if (isCallable(arg)) {
			...
			if (options.globalRules != null) {
				push.apply(globalRules,
					isArray(options.globalRules) ? options.globalRules :
	...
	return mode ? isIgnored(path, mode, Boolean(options && options.watch)).cb(cb) :
		deferred(false).cb(cb);
};
