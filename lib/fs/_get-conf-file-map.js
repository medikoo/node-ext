	...
	updateRoot: function (root) {
		if (!root) {
			...
			this.addPaths(root, this.path)
				.end(this.promise.emit.bind(this.promise, 'change', this.data));
...
getMap = module.exports = function (path, mode, watch, parse) {
	var map = new Map(path, watch), finder, isRoot;
	...
	finder.isRoot = watch ? mode.isRootWatch : mode.isRoot;
	finder.next();
