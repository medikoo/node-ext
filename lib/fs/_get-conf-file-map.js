...
readRules = function (path, filename) {
	return readFile(path + sep + filename)(String, getNull);
};
readRulesWatch = function (path, filename) {
	var watcher, promise, current = null;
	promise = readRules(path, filename);
	watcher = watchPath(path + sep + filename);
	...
	watcher.on('change', function (event) {
		if (event.type === 'remove') {
			...
			readRules(path, filename).end(function (data) {
				if (data !== current) {
...
Map.prototype = {
	...
	addPaths: function (root, path) {
		return deferred.map(paths(root, path), function (path) {
			var rules = this.readRules(path, this.filename);
			if (this.watch) {
				this.rulePromises[path] = rules;
				rules.on('change', this.onRulesChange);
			}
			return rules.end(function (rules) {
				if (rules != null) {
					this[path] = rules;
				}
...
getMap = module.exports = function (path, mode, watch) {
	var map = new Map(path, watch);
	map.filename = mode.filename;
	map.findRoot = watch ? mode.findRootWatch : mode.findRoot;
	return map.init();
};
getMap.Map = Map;
