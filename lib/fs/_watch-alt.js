...
var fs              = require('fs')
  ...
  , watchFile = fs.watchFile, unwatchFile = fs.unwatchFile, readdir

  , other, directory, opts = { persistent: false, interval: 1500 }, rdOpts = {};
...
directory = function (path, emitter) {
	...
	emitter.end = end;
	emitter.close = close;
	readdir(path, rdOpts).end(function (files) {
		data = files.sort();
...
readdir = require('./readdir').readdir;
